...
export class WaypointComponent implements OnInit {
  ...
  inputFRQVar: number | null = 0; //Added Frq variable
  ...
  //Added input frq variable
  set inputFRQ(value: number | null) {
    this.inputFRQVar = value;
  }
  get inputFRQ(): number {
    return this.inputFRQVar ? this.inputFRQVar : this.waypointFRQ();
  }
  ...
  waypointFRQ(): number {
      if (this.waypoint.FRQ != undefined){
          return this.waypoint.FRQ;
      }
      return 0;
  }
  ...
  saveWaypoint() {
    ...
    this.waypoint.FRQ = this.inputFRQ ? this.inputFRQ : undefined;
